...
#type 1 colour of gauge can be changed
            ...
            if (tick%10) == 0:
                self.canvas.create_line(
                    ...
                    *self.to_absolute(outer_point.real, outer_point.imag),
                    width=2,fill='white')                                        #type 1 internal lines color
            else:
                ...
                self.canvas.create_line(
                    ...
                    *self.to_absolute(outer_point3.real, outer_point3.imag),
                    width=1,fill='white')                                       #type 1 internal lines color
            if (tick%10) == 0:
...
#2nd type colour
    ...
    def draw_tick(self, divisions=100):
        ...
        outer_tick_radius = int((self.size-self.size/9) * 0.45)
        label = self.unit
        self.canvas.create_text(self.size/2,3*self.size/10, font=("Arial",int(self.size/20)),fill="white", text=label,angle=0)
        label = ''
        self.canvas.create_text(self.size/2,4*self.size/10, font=("Arial",int(self.size/18),'bold'),fill="white", text=label,angle=0)
        self.readout = self.canvas.create_text(self.size/2,4*self.size/5, font=("Arial",int(self.size/18),'bold'),fill="pink", text='')
        inner_tick_radius2 = int((self.size-self.size/9) * 0.48)
        outer_tick_radius2 = int((self.size-self.size/9) * 0.50)
        inner_tick_radius3 = int((self.size-self.size/9) * 0.35)
        outer_tick_radius3 = int((self.size-self.size/9) * 0.40)
...
#        self.bg_col = bg_col
#        self.unit = '' if not unit else unit
        self.canvas = tk.Canvas(self, width=self.size, height=3 * self.size / 5, bg="black", highlightthickness=0)
        self.canvas.grid(row=0)
        self.readout = self.canvas.create_text(self.size / 2, 4 * self.size / 5,
                                               font=("Arial", int(self.size / 18), 'bold'), fill="white", text='')

    def set_value(self, number: (float, int)):
        label = str('%.2f' % number +"V")
        self.canvas.delete(self.readout)
        self.readout = self.canvas.create_text(140, self.size / 2 + 5,
                                               font=("Arial", int(self.size / 8), 'bold'), fill="white", text=label,
                                               angle=0)
